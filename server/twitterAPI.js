...
const consumer_key = "Qcf3PooY861t6QLhnusU53igN"; // Add your API key here
const consumer_secret = "7cvuTKpJvc52ykOZEMbtjExbgstYZiGej0LZYXoSjJdiRrzbLe"; // Add your API secret key here
...
const trackHashtag = async (hashtag, socket) => {
  console.log("REE");
  let token, currentRules;
  const rules = [{ value: hashtag, tag: hashtag }];
  ...
  const stream = streamConnect(token, socket);
  let timeout = 0;
  stream.on("timeout", () => {
    // Reconnect on error
